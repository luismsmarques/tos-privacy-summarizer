...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadCurrentCredits() {
        try {
            // Usar localStorage em vez de chrome.storage para compatibilidade web
            let result = {};
            try {
                // Tentar chrome.storage primeiro (extensão)
                result = await chrome.storage.local.get(['sharedCredits', 'apiKey', 'userId']);
            } catch (error) {
                // Fallback para localStorage (navegador web)
                result = {
                    sharedCredits: parseInt(localStorage.getItem('sharedCredits')) || 5,
    ...
    // Gerar userId consistente baseado no navegador/dispositivo
    function generateConsistentUserId() {
        // Tentar usar um identificador mais estável
        let deviceId = localStorage.getItem('deviceId');
        
        if (!deviceId) {
            // Gerar um ID baseado em características do navegador
            ...
            deviceId = `web_${btoa(fingerprint).slice(0, 8)}_${timestamp}`;
    ...
    async function createCheckoutSession(packageData) {
        try {
            const response = await fetch('https://tos-privacy-summarizer.vercel.app/api/stripe/create-checkout-session', {
                ...
                body: JSON.stringify({
                    userId: userId || generateConsistentUserId(),
            ...
            // Salvar userId gerado para reutilização
            const finalUserId = userId || generateConsistentUserId();
